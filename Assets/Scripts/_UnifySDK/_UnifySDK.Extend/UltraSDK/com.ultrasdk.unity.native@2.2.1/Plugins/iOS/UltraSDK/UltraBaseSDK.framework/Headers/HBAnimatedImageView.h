...
/**
 ...
 Call `setImage:` with `UIImage(NSImage)` which conform to `HBAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
@interface HBAnimatedImageView : UIImageView
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `HBAnimatedImage` with animated coder which conform to `UltraProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
