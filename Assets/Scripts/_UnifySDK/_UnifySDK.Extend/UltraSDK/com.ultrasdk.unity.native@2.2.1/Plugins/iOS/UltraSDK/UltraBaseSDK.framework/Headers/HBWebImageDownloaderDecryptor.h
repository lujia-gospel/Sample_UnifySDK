...
typedef NSData * _Nullable (^HBWebImageDownloaderDecryptorBlock)(NSData * _Nonnull data, NSURLResponse * _Nullable response);
...
*/
@protocol HBWebImageDownloaderDecryptor <NSObject>
...
/// @note If nil is returned, the image download will be marked as failed with error `HBWebImageErrorBadImageData`
...
*/
@interface HBWebImageDownloaderDecryptor : NSObject <HBWebImageDownloaderDecryptor>

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
- (nonnull instancetype)initWithBlock:(nonnull HBWebImageDownloaderDecryptorBlock)block;

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
+ (nonnull instancetype)decryptorWithBlock:(nonnull HBWebImageDownloaderDecryptorBlock)block;

@end

/// Convenience way to create decryptor for common data encryption.
@interface HBWebImageDownloaderDecryptor (Conveniences)

/// Base64 Encoded image data decryptor
@property (class, readonly, nonnull) HBWebImageDownloaderDecryptor *base64Decryptor;
