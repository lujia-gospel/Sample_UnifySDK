...
@class HBWebImagePrefetcher;

/**
 A token represents a list of URLs, can be used to cancel the download.
 */
@interface UltraWebImagePrefetchToken : NSObject <HBWebImageOperation>
 ...
 */
@protocol HBWebImagePrefetcherDelegate <NSObject>
 ...
 */
- (void)imagePrefetcher:(nonnull HBWebImagePrefetcher *)imagePrefetcher didPrefetchURL:(nullable NSURL *)imageURL finishedCount:(NSUInteger)finishedCount totalCount:(NSUInteger)totalCount;
 ...
 */
- (void)imagePrefetcher:(nonnull HBWebImagePrefetcher *)imagePrefetcher didFinishWithTotalCount:(NSUInteger)totalCount skippedCount:(NSUInteger)skippedCount;

@end

typedef void(^HBWebImagePrefetcherProgressBlock)(NSUInteger noOfFinishedUrls, NSUInteger noOfTotalUrls);
typedef void(^HBWebImagePrefetcherCompletionBlock)(NSUInteger noOfFinishedUrls, NSUInteger noOfSkippedUrls);

/**
 * Prefetch some URLs in the cache for future use. Images are downloaded in low priority.
 */
@interface HBWebImagePrefetcher : NSObject
 ...
 */
@property (strong, nonatomic, readonly, nonnull) HBWebImageManager *manager;
...
/**
 * The options for prefetcher. Defaults to UltraWebImageLowPriority.
 */
@property (nonatomic, assign) UltraWebImageOptions options;

/**
 * The context for prefetcher. Defaults to nil.
 */
@property (nonatomic, copy, nullable) UltraWebImageContext *context;
 ...
 */
@property (weak, nonatomic, nullable) id <HBWebImagePrefetcherDelegate> delegate;

/**
 * Returns the global shared image prefetcher instance. It use a standalone manager which is different from shared manager.
 */
@property (nonatomic, class, readonly, nonnull) HBWebImagePrefetcher *sharedImagePrefetcher;

/**
 * Allows you to instantiate a prefetcher with any arbitrary image manager.
 */
- (nonnull instancetype)initWithImageManager:(nonnull HBWebImageManager *)manager NS_DESIGNATED_INITIALIZER;

/**
 * Assign list of URLs to let HBWebImagePrefetcher to queue the prefetching. It based on the image manager so the image may from the cache and network according to the `options` property.
 ...
 */
- (nullable UltraWebImagePrefetchToken *)prefetchURLs:(nullable NSArray<NSURL *> *)urls;
 ...
 */
- (nullable UltraWebImagePrefetchToken *)prefetchURLs:(nullable NSArray<NSURL *> *)urls
                                          progress:(nullable HBWebImagePrefetcherProgressBlock)progressBlock
                                         completed:(nullable HBWebImagePrefetcherCompletionBlock)completionBlock;
