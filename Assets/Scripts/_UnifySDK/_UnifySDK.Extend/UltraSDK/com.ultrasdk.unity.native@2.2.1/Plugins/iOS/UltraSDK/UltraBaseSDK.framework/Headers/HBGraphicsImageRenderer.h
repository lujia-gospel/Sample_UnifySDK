...
/**
 ...
 For others (macOS/watchOS or iOS/tvOS 10-), these method use the `HBImageGraphics.h` to implements the same behavior (but without dynamic bitmap support)
*/

typedef void (^SDGraphicsImageDrawingActions)(CGContextRef _Nonnull context);
typedef NS_ENUM(NSInteger, HBGraphicsImageRendererFormatRange) {
    HBGraphicsImageRendererFormatRangeUnspecified = -1,
    HBGraphicsImageRendererFormatRangeAutomatic = 0,
    HBGraphicsImageRendererFormatRangeExtended,
    HBGraphicsImageRendererFormatRangeStandard
};

/// A set of drawing attributes that represent the configuration of an image renderer context.
@interface HBGraphicsImageRendererFormat : NSObject
...
@property (nonatomic) HBGraphicsImageRendererFormatRange preferredRange;
...
@interface HBGraphicsImageRenderer : NSObject
...
/// @param size The size of images output from the renderer, specified in points.
/// @param format A HBGraphicsImageRendererFormat object that encapsulates the format used to create the renderer context.
/// @return An initialized image renderer.
- (nonnull instancetype)initWithSize:(CGSize)size format:(nonnull HBGraphicsImageRendererFormat *)format;
