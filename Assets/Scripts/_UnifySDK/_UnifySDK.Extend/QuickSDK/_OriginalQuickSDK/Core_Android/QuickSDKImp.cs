...
namespace quicksdk
{
	...
	public class QuickSDKImp
	{
		...
		{  
#if UNITY_IOS && !UNITY_EDITOR
			QKGame.onPayy(orderInfo.productId, orderInfo.goodsName, (float) orderInfo.amount, orderInfo.cpOrderID,
				orderInfo.callbackUrl,orderInfo.extrasParams);
#elif UNITY_ANDROID && !UNITY_EDITOR
			QuickUnitySupportAndroid androidSupport = QuickUnitySupportAndroid.getInstance();
		...
		{
			#if UNITY_IOS && !UNITY_EDITOR
			QKGame.onUpdateRole( gameRoleInfo.gameRoleID , gameRoleInfo.gameRoleName, gameRoleInfo.serverID, gameRoleInfo.serverName, gameRoleInfo.gameRoleLevel, gameRoleInfo.vipLevel, gameRoleInfo.gameRolePower);
			#elif UNITY_ANDROID && !UNITY_EDITOR
			QuickUnitySupportAndroid androidSupport = QuickUnitySupportAndroid.getInstance();
